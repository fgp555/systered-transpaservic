<template>
  <div class="page">
    ...
    <section class="main" style="margin-top: -10px; height: auto;">
      <div class="main-top">
        <h1 style="color: aliceblue !important;">TIKETS </h1>
      ...
      <center>
        ...
        <div class="table-container">
          <table class="bordered-table">
            <caption style="color: aliceblue !important;">Listado de TIKETS </caption>
            <thead>
              <tr>
    ...
    async obtenerOrdenesPendientes() {
      try {
        ...
        if (userData && userData.data && userData.data.name) {
          ...
          // Realizar la solicitud GET con el nombre de usuario como parte de la URL
          const response = await axios.get(
            `http://138.197.94.5:3000/api/ordenes/tiketsback/`
          );

          console.log("Datos recibidos del servidor:", response.data); // Agrega este console.log para depurar

          // Asignar las órdenes pendientes al arreglo 'ordenes'
          this.ordenes = response.data;
        } else {
          const errorMessage = "No se encontró el nombre de usuario en la información del usuario en sesión.";
          console.error(errorMessage);
          alert(errorMessage); // Mostrar alerta al usuario
        }
      } catch (error) {
  ...
  computed: {
    filteredOrdenes() {
      ...
      return this.ordenes
        ...
        .map((orden) => ({
          ...orden,
          fecha_viaje: typeof orden.fecha_viaje === 'string' ? orden.fecha_viaje.slice(0, 10) : '',
          fecha: typeof orden.fecha === 'string' ? orden.fecha.slice(0, 10) : '' // Formatear la fecha
        }));
    },
  },
  mounted() {
