...
import axios_baseURL from '../../services/service_baseURL';
...
export default {
  ...
  data() {
    return {
      username: 'admin@transpaservic.com.co',
      password: 'Soporte123',
      error: null,
    };
  ...
  mounted() {
    ...
    console.log(`Username inicial: ${this.username}`);
    console.log(`Password inicial: ${this.password}`);
    // console.log("axios_baseURL", axios_baseURL)
    console.log("baseURL", baseURL)
    console.log("process.env.VUE_APP_API_URL", process.env.VUE_APP_API_URL)

    // Prueba de conectividad con el backend
    axios_baseURL.get("/api/users/getAllT")
      .then(response => {
  ...
  methods: {
    ...
    async handleSubmit(data) {
      ...
      try {
        ...
        const response = await axios.post(`${baseURL}/api/users/loginT`, {
          // const response = await axios.post('https://appsystered.com/api/users/login', {
...
.toggle-button {
  ...
  cursor: pointer;
  margin-left: 10px;
  /* Ajusta el margen para que no esté pegado al input */
}
...
.toggle-label {
  margin-left: 5px;
  /* Ajusta el margen para separar la imagen del texto */
  color: aliceblue;
