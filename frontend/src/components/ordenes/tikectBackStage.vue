<template>
  ...
  <table  border="2">
    ...
    <tr style="background-color: indianred;">
      ...
      </tr>       <input type="text"  value="PENDIENTE" placeholder="PENDIENTE POR REVISIÓN Y APROBACIÓN " v-model="estado" readonly class="input"><br><br>
      ...
      <tr>
        <td>
         ...
         <input type="date" v-model="fecha_nueva">
  
        </td>
        <td>
          <h5 style="color: black; text-align: left;">Prestadora Nueva</h5>
          <select v-model="prestadora2" name="empresa">
        ...
        <td>
          ...
          <input type="number" v-model="valor_nuevo">
  ...
  <button style="background-color: darkgray;" type="submit" class="button">Agregar Nuweva Información</button><br>
        ...
        if (userData) {
            ...
            if (userDataObj && userDataObj.data && userDataObj.data.id) {
                ...
                const ordenData = {
                    ...
                    prestadora2:this.prestadora2,
                    valor_nuevo:this.valor_nuevo,
                };
  
                axios.post(`http://138.197.94.5:3000/api/ordenes/tiketback/${orderId}`, ordenData)
                    .then(response => {
                        ...
                        this.$router.push('/back_tikets');                  // Aquí puedes manejar la respuesta del servidor, como redireccionar a otra página o mostrar un mensaje de éxito
                    })
                    .catch(error => {
